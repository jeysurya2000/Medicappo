...
const DoctorSchedules = () => {
  ...
  const fetchSchedules = async () => {
    try {
      const res = await apiClient.get("/doctor/schedules");
  ...
  const saveToAPI = async (updatedSlots) => {
    try {
      const res = await apiClient.put("/doctor/update-slots", {
        date: activeDate,
        slots: updatedSlots,
      });

      setSchedules((prev) => {
        const others = prev.filter((s) => s.date !== activeDate);
        return [...others, res.data.schedule];
      });
    } catch (error) {
  ...
  // ---------- (All Other Functions: edit, delete, add) ----------
  ...
  const saveSlot = async () => {
    ...
    const updated = [...slots];
    updated[editIndex] = trimmedValue;
